...
(function () {
  ...
  // URL base del backend de leads (configurable con <meta name="elite-api-base">).
  const API_BASE = (function () {
    const m = document.querySelector('meta[name="elite-api-base"]');
    return (m && m.content ? m.content : "/api").replace(/\/+$/, "");
  })();
  ...
  function initForm() {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      const payload = {
        ...
        service: v("#f-service"),
        message: v("#f-msg"),
        company: v("#f-company") // honeypot (debe ir vacío)
      };
      ...
      if (ok) {
        ...
        // Respaldo: si el backend no responde, abrimos WhatsApp para no perder la solicitud.
        window.open(waLink(waMessage(v)), "_blank", "noopener");
      }
    });
  }
